...
# 修改日期:
# 修改原因:
# 依赖的表 :  3366_webbill_dsl_rose_webbill_fht0
# ******************************************************************************
...
def data_init():  
    #过程表数据准备
    #清空可能出现的结果数据（重跑时用到）
    sql=""" alter table t_dw_bbly_oss_tblogin add partition p_%s values in ('%s') """ %(iDate,iDate)
    _exec_sql(sql)
    sql=""" alter table t_dw_bbly_oss_tblogin truncate partition (p_%s) """ %(iDate)
    _exec_sql(sql)
...
def stat():
    ...
    sql="""
    insert table t_dw_bbly_oss_tblogin 
	select %s,worldid, uin,min(ilogintime) as ilogintime,max(ionlinetime) as ionlinetime,
	max(iloginip) as iloginip,max(ilevel) as ilevel,min(iloginway) as iloginway from
    (
	    select 
        0 worldid,
        uin,
        min(actiontime) as ilogintime,
	    0 as ionlinetime,
        max(inet_aton(out_ip)) as iloginip,
        -1 as ilevel,
	    f4 as iloginway  
	    from ieg_tdbank::3366_webbill_dsl_baobao_webbill_fht0 
        where moduleid=1017 and actid=1 and f1=1001
        AND tdbank_imp_date BETWEEN %s00 and %s24
        group by uin,worldid,f4
    )a group by uin,worldid
    """%(iDate,iDate,iDate)
